...
object ZLawsF {
  ...
  object Traversable {
    ...
    abstract class PurityLaw[-CapsF[_[+_]], -CapsG[_[+_]], -Caps[_]](label: String)
      ...
      override def run[R1 <: TestConfig, F[+_]: CapsF, G[+_]: CapsG, A: Caps](
        genF: GenF[R1, F],
        genG: GenF[R1, G],
        gen: Gen[R1, A]
      ): ZIO[R1, Nothing, TestResult] =
    ...
    abstract class NaturalityFusionLaw[-CapsF[_[+_]], -CapsG[_[+_]], -Caps[_]](label: String)
      ...
      override def run[R <: TestConfig, F[+_]: CapsF, G[+_]: CapsG, A: Caps](
        genF: GenF[R, F],
        genG: GenF[R, G],
        gen: Gen[R, A],
      ): URIO[R, TestResult] =
    ...
    abstract class SequentialFusionLaw[-CapsF[_[+_]], -CapsG[_[+_]], -Caps[_]](label: String)
        extends Traversable[CapsF, CapsG, Caps, Any] { self =>
      def apply[F[+_]: CapsF, G[+_]: CapsG, H[+_]: CapsG, A: Caps, B: Caps, C: Caps](
        fa: F[A],
        agb: A => G[B],
        bhc: B => H[C]
      ): TestResult
